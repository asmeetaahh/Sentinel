...
export function IncidentHeader({ incident }: { incident: IncidentDetail }) {
  ...
  return (
    <section className="rounded-xl border border-border bg-card p-6 shadow-sm shadow-black/20">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs font-medium tracking-wide text-muted-foreground uppercase">Incident · {humanizeGroupName(incident.event_type)}</p>
          <h2 className="text-xl font-semibold text-foreground">{incident.incident_id}</h2>
          <p className="mt-1 text-sm text-secondary-foreground">{incident.reason_code.label}</p>
        </div>
      ...
      {incident.priority_reasons.length > 0 && (
        <div className="mt-4 border-t border-border-subtle pt-3">
          <p className="text-xs font-medium tracking-wide text-muted-foreground uppercase">Why this priority</p>
          <ul className="mt-1.5 list-disc space-y-0.5 pl-4 text-xs text-secondary-foreground">
      ...
      <p className="mt-4 border-t border-border-subtle pt-3 text-xs leading-relaxed text-muted-foreground">
        {incident.reason_code.description} {incident.reason_code.taxonomy_disclaimer}
      </p>
      <p className="mt-2 text-xs leading-relaxed text-muted-foreground">{incident.scenario_context.note}</p>
    </section>
  )
}
