...
function DeltaCell({ delta }: { delta: Delta | null }) {
  if (!delta) {
    return <span className="text-xs text-slate-400">Not computed</span>
  }
  if (delta.absolute === 0) {
    return <span className="text-xs font-medium text-slate-400">No change</span>
  }
  const color = delta.absolute > 0 ? 'text-red-600' : 'text-teal-600'
...
function ComparisonRow({
  ...
  return (
    <div className="grid grid-cols-[1.4fr_1fr_1fr_1fr] items-center gap-3 border-b border-slate-100 py-3 last:border-0">
      <span className="text-sm text-slate-500">{label}</span>
      <span className="text-right text-sm font-medium text-slate-600 tabular-nums">{current}</span>
      <span className="text-right text-sm font-semibold text-slate-800 tabular-nums">{simulated}</span>
...
export function SimulationResult({ result }: { result: SimulationResponse }) {
  return (
    <MetricCard title="Simulation result" footer={result.modeled_impact_disclaimer}>
      <div className="flex flex-col gap-5">
        ...
        <div>
          <div className="grid grid-cols-[1.4fr_1fr_1fr_1fr] gap-3 border-b border-slate-200 pb-2 text-[11px] font-medium tracking-wide text-slate-400 uppercase">
            <span>Metric</span>
        ...
        <div>
          <p className="mb-2 text-xs font-medium tracking-wide text-slate-400 uppercase">Controls changed</p>
          <ul className="flex flex-col gap-1">
            {result.controls.map((control) => (
              <li key={control.control_id} className="flex items-center justify-between text-sm text-slate-600">
                <span>{control.label}</span>
                <span className="tabular-nums">
                  {formatPercent(control.baseline_value)} <span className="text-slate-300">&rarr;</span>{' '}
                  <span className="font-medium text-indigo-700">{formatPercent(control.simulated_value)}</span>
                </span>
              </li>
            ))}
          </ul>
        ...
        <p className="text-xs text-slate-400">{result.exposure.simulated.method}</p>
      </div>
    </MetricCard>
  )
}
