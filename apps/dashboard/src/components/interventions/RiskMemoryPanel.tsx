              ...
              return (
                <li key={`${record.intervention_id}-${record.timestamp}-${index}`} className="flex flex-col gap-1.5 py-3 first:pt-0 last:pb-0">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <span className="text-sm font-medium text-foreground">{record.recommendation_title}</span>
                    <span className="text-[11px] text-muted-foreground tabular-nums">{formatDateTime(record.timestamp)}</span>
                  </div>
                  ...
                  <p className="text-[11px] text-muted-foreground">{record.outcome_note}</p>
                </li>
              )
            })}
        </ul>
