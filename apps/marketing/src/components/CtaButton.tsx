 ...
 * everywhere it appears. No route exists yet for it to go to (see
 * Nav/Hero), so it renders as a real, styled anchor without a working
 * destination rather than an invented page. */
export function CtaButton({ variant = 'primary', className = '', children, ...props }: CtaButtonProps) {
  ...
  return (
    <a href="#" onClick={(event) => event.preventDefault()} className={`${base} ${styles} ${className}`} {...props}>
      {children}
